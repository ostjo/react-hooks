...
- DOM nodes are only created once `ReactDOM.render` is called
- Once the component is mounted, the component's ref should have a `current` (so
  usually DOM manipulations are placed inside the `useEffect` callback)
- Remember to clean up (otherwise there might be event handlers dangling around
  on DOM nodes that are no longer in the document)
