...
The `useEffect` hook will be called every time the component renders (and
re-renders).

### Lazy state initialization

`useState` can not only be initialized with a value, but can also take in a
**callback function**. That function will only be called the **first time the
component is rendered**.

For example, when initializing `useState` with a value read from `localStorage`,
we could use lazy state initialization to only run this expensive operation on
initial render (reading from `localStorage` can be a performance bottleneck).
...
we can simply initialize like this:
