...
There's two ways of handling a promise error:
...
**→ Option 1:** safe option, since it catches both errors of `setPokemon` and
`fetchPokemon`.

**→ Option 2:** more precise option, since it only catches errors of
`fetchPokemon`.

### State enums instead of booleans

It's [encouraged](https://kentcdodds.com/blog/stop-using-isloading-booleans) to
use `status` enums instead of booleans like `isLoading`.
...
React normally batches `setState` calls, but it's unable to do this in an
asynchronous callback (like e.g. promise handlers) – at least up until **React
18**.

Still, it is better to **maintain closely related states as an object** rather
than maintaining them using individual `useState` hooks.

Another elegant way to approach this issue is `useReducer`, which will be
covered in a later chapter.

### Error boundaries

We can wrap parts of our application in Error boundaries. Besides using
`ErrorBoundary` libs, it's also possible to write your own `ErrorBoundary`
component. **⚠️ But: `ErrorBoundary` can only be written as a class component,
not a function component.**

[Here's](https://react.dev/reference/react/Component#catching-rendering-errors-with-an-error-boundary)
the React docs on error boundaries.

> **💡 Tip:** Give the custom `ErrorBoundary` component a `key` to allow
> resetting of its state if the `key` changes.

A common package is `react-error-boundary`, however, it's important to note that
React doesn't hand off all errors to error boundaries, like:

- Event handlers
- Async code (e.g. `setTimeout` or `requestAnimationFrame` callbacks)
- SSR
- Errors in the boundary component itself (not thrown by `children`)

To address this, `react-error-boundary` comes with a `useErrorBoundary` hook and
its `showBoundary` function, that propagates the error to the nearest boundary:
...
Check out this
[article](https://kentcdodds.com/blog/use-react-error-boundary-to-handle-errors-in-react#handle-all-errors)
for more info.

`react-error-boundary` can be applied in the following way:

1. Wrap the desired part of the app in the lib's `ErrorBoundary` component.
2. Pass a `FallbackComponent` to it that it will show in case of an error.
3. The `FallbackComponent` has access to a method called `resetErrorBoundary`,
   which will trigger a reset of the error boundary when called.
4. Provide a `onReset` function to the `ErrorBoundary` that will execute once
   the boundary is reset.
5. _Optional:_ Provide an array of keys to `resetKeys`, which behaves like a
   dependencies array: If one of the keys changes, the boundary will reset
   itself.
